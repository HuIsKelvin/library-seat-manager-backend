...
def SeatReleaseOverTime():
	seatUpdate="update seat_info set seat_status=0, user_id=-1 where seat_id=(select seat_id from seat_leave_briefly where leave_time < (select datetime('now','localtime','-60 minutes')));"
	seatRelease="delete from seat_leave_briefly  where leave_time < (select datetime('now','localtime','-60 minutes'));"
	con=sql.connect("feedback.db")
	cursor=con.cursor()
...
'''
schedule.every(3).seconds.do(SeatReleaseOverTime)
while True:
